    ...
    <section id="projects" ref={ref} className=" bg-gray-900 body-font">
      <div className="container px-5 py-20 mx-auto text-center lg:px-40">
        <div className="flex flex-col w-full mb-20 bg-orange-700 shadow-lg shadow-black py-10 px-10">
          <h1 className={`md:text-3xl text-l font-bold title-font mb-4 text-black pb-10 pt-6 ${inView ? 'animate-slidein100 opacity-0' : 'opacity-0'}`}>
          -- - --   PROJECTS   -- - --
          </h1>
          <p className={`text-left text-black lg:w-2/3 mx-auto leading-relaxed text-l md:text-2xl md:font-extrabold ${inView ? 'animate-slidein300 opacity-0' : 'opacity-0'}`}>
        ...
        <div className="flex flex-wrap -m-4">
          {projects.map((project) => (
            <a
              ...
              <h2 className="text-orange-500 md:py-12 py-6 md:text-4xl text-lg">{project.title}</h2>
              ...
              <hr className="border-t-2 border-orange-500 md:my-20 md:mx-20 my-5" />
            </a>
          ))}
        </div>
      </div>
